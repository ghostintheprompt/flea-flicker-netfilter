...
echo -e "${YELLOW}Version 3.0 - Flea Flicker Edition (Trick Play Mode)${NC}"
echo ""
...
show_usage() {
    ...
    echo -e "  ${BLUE}ai-evasion${NC}     - Advanced AI detection evasion"
    ...
    echo -e "  ${PURPLE}ghost${NC}          - Ultimate stealth (blocks all AI/telemetry)"
    echo -e "  ${PURPLE}shadow${NC}        - Red team + AI evasion + Tor routing"
    echo -e "  ${PURPLE}phantom${NC}       - Maximum stealth + traffic mimicry"
    echo ""
    echo -e "${GREEN}🏈 Experimental Flea Flicker Modes:${NC}"
    echo -e "  ${RED}trick-play${NC}     - Basic MAC spoofing + AI confusion"
    echo -e "  ${RED}misdirection${NC}   - Advanced MAC rotation + honeypots"
    echo -e "  ${RED}deception${NC}      - Full deception suite + Web3 attacks"
    ...
    echo -e "  ${CYAN}./launch.sh ai-evasion${NC}              # Block AI services"
    echo -e "  ${CYAN}./launch.sh ghost${NC}                   # Ultimate stealth"
    echo -e "  ${CYAN}./launch.sh redteam --interactive${NC}   # Interactive red team mode"
    echo -e "  ${CYAN}./launch.sh trick-play${NC}              # 🏈 Basic Flea Flicker mode"
    echo -e "  ${CYAN}./launch.sh chaos${NC}                   # 🏈 Maximum experimental chaos"
    echo ""
}
...
run_mode() {
    ...
    case "$mode" in
        ...
        "basic")
            ...
            sudo python3 netfilter.py $extra_args
            ;;
        "stealth")
            echo -e "${GREEN}[*] Starting Stealth Mode${NC}"
            sudo python3 netfilter.py --stealth --stealth-mode basic $extra_args
            ;;
        "ai-evasion")
            echo -e "${GREEN}[*] Starting AI Evasion Mode${NC}"
            echo -e "${YELLOW}[*] Loading AI detection evasion rules...${NC}"
            sudo python3 netfilter.py --ai-evasion --stealth-mode advanced \
        ...
        "maximum-stealth")
            ...
            sudo python3 netfilter.py --stealth --stealth-mode maximum \
                --config advanced_stealth_rules.json $extra_args
            ;;
        "kali")
            echo -e "${GREEN}[*] Starting Kali Linux Mode${NC}"
            sudo python3 netfilter.py --config kali_rules.json $extra_args
            ;;
        "redteam")
            echo -e "${GREEN}[*] Starting Red Team Mode${NC}"
            sudo python3 netfilter.py --config redteam_rules.json $extra_args
            ;;
        "ghost")
            echo -e "${PURPLE}[*] 👻 GHOST MODE ACTIVATED${NC}"
            echo -e "${YELLOW}[*] Ultimate stealth - blocking all AI/telemetry${NC}"
            sudo python3 netfilter.py --ai-evasion --stealth-mode maximum \
                --config advanced_stealth_rules.json --stealth $extra_args
            ;;
        "shadow")
            echo -e "${PURPLE}[*] 🕴️  SHADOW MODE ACTIVATED${NC}"
            echo -e "${YELLOW}[*] Red team + AI evasion + Tor routing${NC}"
            # Check if Tor is available
            if command -v tor &> /dev/null; then
                echo -e "${GREEN}[✓] Tor service detected${NC}"
                sudo python3 netfilter.py --config redteam_rules.json \
        ...
        "deception")
            ...
            sudo python3 netfilter.py --flea-flicker maximum \
                --config advanced_stealth_rules.json --ai-evasion --stealth $extra_args
            ;;
        "chaos")
            echo -e "${RED}[*] 🏈 FLEA FLICKER: MAXIMUM CHAOS MODE ACTIVATED${NC}"
            echo -e "${YELLOW}[*] ⚠️  ALL EXPERIMENTAL FEATURES ENABLED ⚠️${NC}"
            echo -e "${RED}[*] MAC rotation, AI traps, Web3 attacks, MitM feints${NC}"
            echo -e "${CYAN}[*] Entering maximum chaos mode...${NC}"
            sudo python3 netfilter.py --flea-flicker chaos \
                --config advanced_stealth_rules.json --ai-evasion --stealth $extra_args
            ;;
        *)
...
main() {
    ...
    case "$mode" in
        "ghost"|"shadow"|"phantom"|"ai-evasion"|"maximum-stealth")
            echo -e "${RED}⚠️  WARNING: Advanced stealth mode activated${NC}"
            echo -e "${YELLOW}   This will block AI services and may affect normal browsing${NC}"
            echo -e "${YELLOW}   Press Ctrl+C to cancel, or Enter to continue...${NC}"
            read -t 5 || true
            echo ""
            ;;
        "trick-play"|"misdirection"|"deception"|"chaos")
            echo -e "${RED}🏈 ⚠️  EXPERIMENTAL FLEA FLICKER MODE WARNING ⚠️${NC}"
            echo -e "${YELLOW}   This mode uses EXPERIMENTAL evasion techniques:${NC}"
            echo -e "${RED}   • MAC address spoofing and rotation${NC}"
            echo -e "${RED}   • AI trap honeypots and confusion tactics${NC}"
            echo -e "${RED}   • Simulated Web3 attacks and MitM feints${NC}"
            echo -e "${RED}   • Protocol confusion and emotional AI manipulation${NC}"
            echo ""
